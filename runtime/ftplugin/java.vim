...
if (!empty(get(g:, 'spotbugs_properties', {})) ||
    ...
    function! g:JavaExecuteActionOnce(cleanup_cmd, action_cmd) abort
	try
	...
	for s:action in s:actions
		...
		execute printf('autocmd java_spotbugs %s <buffer> ' .
			\ 'call g:JavaExecuteActionOnce(%s, %s)',
		    \ s:action.event,
	...
	if s:SpotBugsHasProperty('PostCompilerActionExecutor') &&
	    ...
	    if !exists("#java_spotbugs_post")
		augroup java_spotbugs_post
		augroup END
	    endif
	    ...
	    " it decide whether to proceed with ":compiler spotbugs" etc.
	    execute printf('autocmd java_spotbugs_post User <buffer> ' .
				\ 'call g:JavaExecuteActionOnce(%s, %s)',
		\ string('autocmd! java_spotbugs_post ShellCmdPost <buffer>'),
		\ string(printf('autocmd java_spotbugs_post ShellCmdPost <buffer> ' .
				\ 'call g:JavaExecuteActionOnce(%s, %s)',
		    \ string('autocmd! java_spotbugs_post ShellCmdPost <buffer>'),
...
function! JavaFileTypeCleanUp() abort
    ...
    " (Note that a ShellCmdPost definition must not be removed, and yet it
    " need not be removed on account of its once-only attribute.)
    silent! autocmd! java_spotbugs User <buffer>
    silent! autocmd! java_spotbugs Syntax <buffer>
    silent! autocmd! java_spotbugs_post User <buffer>
endfunction
